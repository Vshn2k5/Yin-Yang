...
import { authAPI, type OAuthProvider } from '../lib/auth';
...
const oauthProviders: OAuthProvider[] = ['google', 'github', 'facebook', 'twitter'];
...
const AuthPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessage({ type: 'error', text: getErrorMessage(error, 'An error occurred') });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleOAuthSignIn = async (provider: OAuthProvider) => {
    if (!enabledOAuthProviders.has(provider)) {
      setMessage({
        type: 'info',
        text: `${oauthLabels[provider]} sign-in is not enabled in Supabase yet. Use email sign-in or enable the provider in Supabase first.`
      });
      return;
    }
    ...
    try {
      ...
      if (!data?.url) {
        throw new Error(`No ${oauthLabels[provider]} sign-in URL was returned`);
      }
      ...
      setMessage({
        type: 'error',
        text: getErrorMessage(error, `${oauthLabels[provider]} sign in failed`)
      });
  ...
  const renderOAuthButtons = () => (
    <div className="space-y-3">
      <button
        ...
        title={enabledOAuthProviders.has('google') ? 'Continue with Google' : 'Google provider is not enabled in Supabase'}
        className={`w-full flex items-center justify-center px-4 py-3 border border-gray-600 rounded-lg bg-gray-700 text-white transition-colors disabled:opacity-50 ${
          enabledOAuthProviders.has('google') ? 'hover:bg-gray-600' : 'opacity-60 cursor-help'
        }`}
      >
        {oauthLoading === 'google' ? <Loader2 className="w-5 h-5 mr-3 animate-spin" /> : <Chrome className="w-5 h-5 mr-3" />}
      ...
      <button
        ...
        disabled={isBusy}
        aria-disabled={isBusy}
        title={enabledOAuthProviders.has('github') ? 'Continue with GitHub' : 'GitHub provider is not enabled in Supabase'}
        className={`w-full flex items-center justify-center px-4 py-3 border border-gray-600 rounded-lg bg-gray-700 text-white transition-colors disabled:opacity-50 ${
          enabledOAuthProviders.has('github') ? 'hover:bg-gray-600' : 'opacity-60 cursor-help'
        }`}
      >
        {oauthLoading === 'github' ? <Loader2 className="w-5 h-5 mr-3 animate-spin" /> : <Github className="w-5 h-5 mr-3" />}
      ...
      <div className="grid grid-cols-2 gap-3">
        <button
          ...
          disabled={isBusy}
          aria-disabled={isBusy}
          title={enabledOAuthProviders.has('facebook') ? 'Continue with Facebook' : 'Facebook provider is not enabled in Supabase'}
          className={`flex items-center justify-center px-4 py-3 border border-gray-600 rounded-lg bg-blue-600 text-white transition-colors disabled:opacity-50 ${
            enabledOAuthProviders.has('facebook') ? 'hover:bg-blue-700' : 'opacity-60 cursor-help'
          }`}
        >
          {oauthLoading === 'facebook' ? <Loader2 className="w-5 h-5 mr-2 animate-spin" /> : <Facebook className="w-5 h-5 mr-2" />}
          Facebook
        </button>
        ...
        <button
          ...
          disabled={isBusy}
          aria-disabled={isBusy}
          title={enabledOAuthProviders.has('twitter') ? 'Continue with Twitter' : 'Twitter provider is not enabled in Supabase'}
          className={`flex items-center justify-center px-4 py-3 border border-gray-600 rounded-lg bg-blue-400 text-white transition-colors disabled:opacity-50 ${
            enabledOAuthProviders.has('twitter') ? 'hover:bg-blue-500' : 'opacity-60 cursor-help'
          }`}
        >
          {oauthLoading === 'twitter' ? <Loader2 className="w-5 h-5 mr-2 animate-spin" /> : <Twitter className="w-5 h-5 mr-2" />}
          Twitter
        </button>
      </div>
    </div>
